...
Avocados. They are a hearty, savory and delicious snack. In the recent decade, avocados have trended harder than alternative dairy and meat products, and yet, the humble fruit does not aim to change the world. In multi-part post, I dive in to the pit of the avocado boom during in the last half of the 2010s. I explore a dataset, modeling techniques, and R dashboarding summarizations. 

These posts are intended for the data analyst or data scientist who use R. The final dashboard is intended for a broad audience, but should be read with the same mindset a cafe manager would have. More on that in a later post.
...
We start the project by pulling a user-compile dataset from Kaggle, and credit is due to [Justin Kiggins](https://www.kaggle.com/neuromusic). Below is a summary from the dataset [source page]https://www.kaggle.com/neuromusic.
